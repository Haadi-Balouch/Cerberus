# ============================
# Cerberus - Professional Security Report Generator
...
def build_cve_section(story, all_cves):
    ...
    for sev in ["CRITICAL", "HIGH", "MEDIUM", "LOW"]:
        ...
        for cve in cves:
            ...
            cve_data = [
                [Paragraph(f"<b>{cve['cve']}</b>", BodyText), ""],
...
def generate_pdf(data):
    target = data["target"]
    filename = f"outputs/report_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.pdf"
    ...
    build_recon_section(story, data["recon"])
    build_port_section(story, data["ports"])
